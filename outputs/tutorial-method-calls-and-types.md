...
| `[tstr]` | `std::vector<std::string>` | `serde_json::Value` |
| `[int]` | `std::vector<int64_t>` | `serde_json::Value` |
| `[uint]` | `std::vector<uint64_t>` | `serde_json::Value` |
| `[float64]` | `std::vector<double>` | `serde_json::Value` |
| `[bool]` | `std::vector<bool>` | `serde_json::Value` |
| `[any]` | `LogosList` | `serde_json::Value` |
| `{tstr: any}` | `LogosMap` | `serde_json::Value` |
| `[bstr]` | `std::vector<std::vector<uint8_t>>` | `serde_json::Value` |
| `{tstr: bstr}` | `std::map<std::string, std::vector<uint8_t>>` | `serde_json::Value` |
| `{tstr: int}` | `std::map<std::string, int64_t>` | `serde_json::Value` |
| `{tstr: int}` | `std::unordered_map<std::string, int64_t>` | `serde_json::Value` |
| `[[int]]` | `std::vector<std::vector<int64_t>>` | `serde_json::Value` |
| `Entry` | `Entry` | `Entry` |
| `[Entry]` | `std::vector<Entry>` | `Vec<Entry>` |
| `{tstr: Entry}` | `std::map<std::string, Entry>` | `serde_json::Value` |
| `Batch` | `Batch` | `Batch` |
...
map contracts and rejects optional returns. Declaring the LIDL directly exposes
the larger supported surface and generates the trait you implement. Do not write
`Vec<i64>` in that generated provider impl: general collections currently arrive
as `serde_json::Value`; a record and `[Record]` are the typed exceptions. Record
**fields** are typed recursively (for example `Batch.tags: Vec<String>`).
...
fields are encoded by the SDK; when a Rust collection arrives as `Value`, retain
those tags. A list of JSON integers is not the canonical byte representation.

Generated **clients** need not have the same collection signatures as providers:
C++ wrappers use `LogosList`/`LogosMap` for many general collections; Rust clients
borrow `&serde_json::Value`, return `Value`, and borrow typed records. Inspect
`nix build .#generate` when you need the exact generated signature.
